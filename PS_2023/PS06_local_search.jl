...
The overview of the algorithm is shown below
  ...
  $(PlutoUI.LocalResource("./img/MDProbot.pdf", :width => 400, :align => "center"))
...
Let's try write a message by using a genetic algorithm. E.g "173 Pol evolved!" starting from a random set of characters (a-zA-Z0-9. !)
"""
...
begin
	...
	const pop_size = 100
	const goal = collect("173 Pol evolved!");
	...
	"""
	function fitness_message(m::Array{Char,1}; goal=goal)
		sum(m.==goal)
	end
	nothing
end
...
let
	...
	for i = 1:n
		duration[i],_,_ = textga(ngen=500, popsize=100, pmut=0.1)
	end

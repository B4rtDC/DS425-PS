...
function policy_extraction(mdp::T, U::Dict) where T<:AbstractMarkovDecisionProcess
	...
	for state in mdp.states
		...
		Π[state] = A[argmax(expected_utility)]
	end
	return Π
end
...
"""
    policy_evaluation(Π::Dict, U::Dict, mdp::T; k::Int64=20) where {T <: AbstractMarkovDecisionProcess}

Return the updated utilities of the MDP's states by applying the modified policy iteration
algorithm on the given Markov decision process 'mdp', utility function 'U', policy 'Π',
and number of Bellman updates to use 'k'.
"""
function policy_evaluation(Π::Dict, U::Dict, mdp::T; k::Int64=20) where {T <: AbstractMarkovDecisionProcess}
    for i in 1:k
        for state in mdp.states
            U[state] = reward(mdp, state) + mdp.gamma * sum(p * U[newstate] for (p, newstate) in transition_model(mdp, state, Π[state]));
        end
    end
    return U;
end

# function policy_evaluation(Π::Dict, U::Dict, gmdp::GridMarkovDecisionProcess; k::Int64=20)
#     for i in 1:k
#         for state in gmdp.states
#             U[state] = reward(gmdp, state) + gmdp.gamma * sum(p * U[newstate] for (p, newstate) in transition_model(gmdp, state, Π[state]));
#         end
#     end
#     return U;
# end

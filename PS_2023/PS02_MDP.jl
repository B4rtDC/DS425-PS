...
Below different solutions for the first three questions are shown:
  ...
  $(robotsolution ? PlutoUI.LocalResource("./img/cleaningrobotMDP.png", :width => 300, :align => "center") : "")
$(humansolution ? PlutoUI.LocalResource("./img/humanMDP.png", :width => 500, :align => "center") : "")
$(lecturesolution ? PlutoUI.LocalResource("./img/lecturesolution.png", :width => 500, :align => "center") : "")
...
In the additional file ("PS14\_MDP\_spt.jl"), you can find a generic implentation for dealing with MDP's. The contents of this file can be loaded by using the helper function `ingredients` (hidden in the cell below).
"""

# ╔═╡ 282511b7-ccd6-4771-ac37-3c76751d6ff0
begin
...
# ╔═╡ 1b9e15c6-35f0-4e22-9238-7346db7f035a
md"""
The implementation of two solution methods are given below.
"""
...
MDP = ingredients("./PS14_MDP_spt.jl");nothing
...
# ╔═╡ 4b3b3fe0-79ef-4823-aa4d-ace6a220497a
begin
	"""
	...
	"""
		policy_extraction(mdp::T, U::Dict)

	Return the policy from a MDP given the computed utilities.
	"""
...
tt = transition_model(myMDP,pos,action)
...
let
	state = (1,4)
	[(action, transition_model(myMDP,state,action)) for action in actions(myMDP,state)]
end
...
pol = policy_extraction(myMDP, VI)
...
polbis,ubis = policy_iteration(myMDP)

# ╔═╡ 01b0512d-c18d-4d68-ba46-4909b311bbc6
*(vcat("Dict(",join(["$(key) => $(val)" for (key,val) in polbis],", "), ")")...)
...
Questions:
1. make a graphical representation of this problem (states, transition model, reward)

...
3. Implement a performance-measuring environment simulator for the vacuum-cleaner world. Think about a modular implementation so that the sensors, actuators, and environment characteristics (size, shape, dirt placement, etc.) can be changed easily. 
...
begin
	...
	# include the actions
	# set the rules (only two options)
	...
	function update!(v::reflex_vacuum)
		...
		if status == true # if dirty => clean
			...
			pos = rules[pos] # `rules` is a defined constant
			v.percept = (pos, v.w.locations[pos]) # update sensor percept
		end
	end

	"""
		runprog(v::reflex_vacuum, n::Int=1000)

	Run a small-world reflex vacuum `n` times
	"""
...
begin
	# actual simulation
	@info "Starting..."
	for layout in Base.Iterators.product([true, false],[true, false]) # run all possible combinations
		for startpos in [(0,0),(1,0)] # and starting locations
			# initiate the world
			w = world([(0,0),(1,0)],[i for i in layout])
			# initiate reflex_vacuum
			v = reflex_vacuum(w, (startpos, w.locations[startpos]))
			# scoring
			score = runprog(v,10) # limited to 10 to avoid logmsg overload
			# output msg

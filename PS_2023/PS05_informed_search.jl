...
# ╔═╡ 7e539d1d-8b28-4649-b376-68a800a65e9a
using Graphs, SimpleWeightedGraphs, Plots, LinearAlgebra
...
begin
	...
	goaltest_Robot(p::RobotProblem, n::RobotNode; kwargs...) = sqrt((n.state[1] - p.goalstate[1])^2 + (n.state[1] - p.goalstate[1])^2 )  <=  p.tolpos
...
let
	prob = RobotProblem( (0., 0.), (30., 20.),Δx=0.5, Δy=1.)
	...
	xlabval = collect(prob.rootnode.state[1]:prob.Δx:prob.goalstate[1])
	xticks!(xlabval, ["$(v)" for v in xlabval])
	#yticks!(collect(1:20:length(y)), ["$(v)" for v in collect(x)[1:20:length(y)]])
	#prob.rootnode.state[1]:prob.Δx:prob.goalstate[1]
end

# ╔═╡ ab3c24a8-5bd1-4bdb-8ab3-2bc8d993e5f3
...
# ╔═╡ d610a351-4db3-4579-9706-f861e14e194f
ff.path

# ╔═╡ e9123312-83eb-44ea-a931-19692333e079


# ╔═╡ 6c600b61-cfbf-4f21-9bbc-7feeb4773be4
qq

# ╔═╡ 3f79bf19-052b-45ce-b1bf-6e2b43cdd7ba
# possible actions based on possible moves
	


# ╔═╡ 6f173759-3607-40f6-aeb6-18e52f0bb6c3


# ╔═╡ be3d571c-6ef4-4c2b-b232-6e3d4ce319a6


# ╔═╡ 04ea8b60-fcd5-4b10-9d22-93498f32ead8


# ╔═╡ 2f7c8352-0d63-4010-82da-8a956fd98811


# ╔═╡ 2eacb738-bad1-4a4a-ab4d-07ebd541b76b


# ╔═╡ 6b87c8f1-8ea1-4eae-9569-12015e7802be
begin
	rr = RobotNode((1.,4.), nothing, 0., 0)
	pp = RobotProblem(rr, (3.,6.), Float64[0; 7;0;7],1.,1.,1)
	findactionsRobot(pp,rr)
end

# ╔═╡ 00ac4318-bda9-433b-9520-cb950601ef81


# ╔═╡ e8ef9542-d628-4961-971a-03af5e9f3883


# ╔═╡ cfe929ba-f530-4352-aa23-5a96baf21e8d


# ╔═╡ 336ff7b6-dfdb-4d10-b136-c5df746c7b99


# ╔═╡ 8909e05a-166f-41aa-81a4-94fde6155dd7


# ╔═╡ de809d9f-f494-4410-a540-c25cdbf3760b
